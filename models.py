...
def encoder(opts, inputs, reuse=False, is_training=False, y=None):
...
def decoder(opts, noise, reuse=False, is_training=True):
    ...
    with tf.variable_scope("generator", reuse=reuse):
        if opts['mlp_classifier']:
            ...
            out = tf.nn.relu(out)
            probs = ops.linear(opts, out, opts['n_classes'], 'classifier')
        else:
            probs = ops.linear(opts, noise, opts['n_classes'], 'classifier')
